...
internal static class DriveBenderExtensions {
  ...
  public static IEnumerable<DriveBender.IPhysicalFile> EnumerateFiles(this IEnumerable<DriveBender.IPhysicalFileSystemItem> items, bool suppressExceptions = false) {
    var emptyFileSystemItems = Array.Empty<DriveBender.IPhysicalFileSystemItem>();
...
public static partial class DriveBender {
  ...
  public class SizeFormatter {
    ...
    private static readonly (ulong, string)[] SIZE_VALUES = [
      (EiB, "EiB"), 
      (PiB, "PiB"), 
      (TiB, "TiB"), 
      (GiB, "GiB"), 
  ...
  public static class DriveBenderConstants {
  ...
  public partial class MountPoint : IMountPoint {
  ...
  public class PoolDriveWithoutPool(string label, string description, Guid id, DirectoryInfo root) {

    private readonly DirectoryInfo _root = root ?? throw new ArgumentNullException(nameof(root));
    private readonly string _description = description;
    
    private string _Name => this._root.Parent?.Name;
    public Guid Id { get; } = id;

    public Volume AttachTo(IMountPoint mountPoint) => new(mountPoint, label, this._description, this.Id, this._root);

  }

  [DebuggerDisplay("{Root.FullName}:{" + nameof(Label) + "}")]
  public class Volume(IMountPoint mountPoint, string label, string description, Guid id, DirectoryInfo root)
    : IVolume, IEquatable<Volume> {
    ...
    public IMountPoint MountPoint { get; } = mountPoint;
  ...
  private class PhysicalFolder(Volume drive, DirectoryInfo physical, PhysicalFolder parent)
    : IPhysicalFolder {

    private readonly DirectoryInfo _physical = physical ?? throw new ArgumentNullException(nameof(physical));
    private readonly Volume _drive = drive ?? throw new ArgumentNullException(nameof(drive));
    ...
    public IPhysicalFolder Parent => parent;
  ...
  [DebuggerDisplay("{" + nameof(FullName) + "}")]
  private class PhysicalFile(FileInfo physical, PhysicalFolder parent, bool isShadowCopy)
    : IPhysicalFile {
    #region Implementation of IFileSystemItem

    public FileInfo Source { get; } = physical ?? throw new ArgumentNullException(nameof(physical));
    ...
    public bool IsShadowCopy { get; } = isShadowCopy;
